require('../css/audio-player.scss');

/*
Example: https://www.codehim.com/demo/javascript-audio-player-with-playlist/
*/

export var ap_simp, simp_config, simp_auto_load, simp_audio, simp_album, simp_cover, simp_title, simp_artist, simp_controls, simp_progress, simp_volume, simp_v_slider, simp_v_num, simp_others, simp_playlist, simp_source, simp_a_url, simp_a_index, simp_isPlaying, simp_isNext, simp_isRandom, simp_isRanext, simp_isStream, simp_isLoaded;
...
function simp_initTime() {
  ...
  if (simp_audio.currentTime == simp_audio.duration) {
    simp_controls.querySelector('.simp-plause').classList.remove('bi-pause-fill');
    simp_controls.querySelector('.simp-plause').classList.add('bi-play-fill');
...
function simp_setAlbum(index) {
    simp_cover.innerHTML = simp_a_url[index].dataset.cover ? '<div style="background:url(' + simp_a_url[index].dataset.cover + ') no-repeat;background-size:cover;width:80px;height:80px;"></div>' : '<i class="bi bi-music-note-beamed display-2"></i>';
...
function simp_changeAudio(elem) {
    ...
    if (simp_isPlaying) {
      simp_controls.querySelector('.simp-plause').classList.remove('bi-play-fill');
      simp_controls.querySelector('.simp-plause').classList.add('bi-pause-fill');
    }
}
  ...
  simp_controls.querySelector('.simp-plauseward').addEventListener('click', function(e) {
    ...
    if (eles.contains('simp-plause')) {
      if (simp_audio.paused) {
        ...
        simp_audio.play();
        simp_isPlaying = true;
        eles.remove('bi-play-fill');
        eles.add('bi-pause-fill');
      } else {
        simp_audio.pause();
        simp_isPlaying = false;
        eles.remove('bi-pause-fill');
        eles.add('bi-play-fill');
      }
    } else {
  ...
  simp_volume.addEventListener('click', function(e) {
    ...
    if (eles.contains('simp-mute')) {
      if (eles.contains('bi-volume-up-fill')) {
        ...
        eles.add('bi-volume-up-fill');
        simp_v_slider.value = simp_v_num;
      }
    } else {
      simp_v_num = simp_v_slider.value;
      if (simp_v_num != 0) {
        simp_controls.querySelector('.simp-mute').classList.remove('bi-volume-mute-fill');
        simp_controls.querySelector('.simp-mute').classList.add('bi-volume-up-fill');
      }
    }
...
// Start simple player
export function start_audio_player()
{
    if (document.querySelector('#simp')) {
        ap_simp = document.querySelector('#simp');
        simp_playlist = ap_simp.querySelector('.simp-playlist');
        simp_source = simp_playlist.querySelectorAll('li');
        ...
        simp_config = ap_simp.dataset.config ? JSON.parse(ap_simp.dataset.config) : {
        ...
        simp_elem += '<div class="simp-display"><div class="simp-album w-full flex-wrap"><div class="simp-cover"><i class="bi bi-music-note-beamed display-2"></i></div><div class="simp-info"><div class="simp-title">Title</div><div class="simp-artist">Artist</div></div></div></div>';
        simp_elem += '<div class="simp-controls flex-wrap flex-align">';
        simp_elem += '<div class="simp-plauseward flex flex-align"><button type="button" class="simp-prev bi bi-rewind-fill" disabled></button><button type="button" class="simp-plause bi bi-play-fill" disabled></button><button type="button" class="simp-next bi bi-fast-forward-fill" disabled></button></div>';
...
window.audioPlayer = start_audio_player;
